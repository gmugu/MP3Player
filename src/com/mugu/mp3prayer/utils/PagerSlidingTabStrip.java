 ...
 * 
 * 
 * ViewPage的指示界面，第三方控件，不需要理解，会用就行
 */
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int tabTextColorOnSelected = -1;
		...
		if (setupListener) {
			pager.setOnPageChangeListener(new OnPageChangeListener() {
				...
				public void onPageScrolled(int position, float positionOffset,
					...
					PagerSlidingTabStrip.this.onPageScrolled(position,
	 ...
	 * author:zhaobinbin
	 */
	private void updateTextColorOnSelected(int position) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				if (i == position) {
					tab.setTextColor(tabTextColorOnSelected);
				} else {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				if (tabTextColorStateListId != -1) {
					ColorStateList cs = getResources().getColorStateList(
							tabTextColorStateListId);
					tab.setTextColor(cs);
				} else {
					tab.setTextColor(tabTextColor);
				}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRect(lineLeft, height - indicatorHeight - underlineHeight,
	...
	public void onPageSelected(int position) {
		if (tabTextColorOnSelected != -1) {
			updateTextColorOnSelected(position);
		}
	}
	 ...
	 * 
	 * */
	public void setTextColorOnSelected(int textColor) {
		this.tabTextColorOnSelected = textColor;
		updateTextColorOnSelected(currentPosition);
	}
